...
if ( typeof define === 'function' && define.amd ) {
  ...
  window.IsotopeMB.Item = itemDefinition(
    window.Outlayer
  );
}
...
if ( typeof define === 'function' && define.amd ) {
  ...
  // browser global
  window.IsotopeMB = window.IsotopeMB || {};
  window.IsotopeMB.LayoutMode = layoutModeDefinition(
    window.getSize,
    window.Outlayer
  );
}
...
function masonryDefinition( LayoutMode, Masonry ) {
  ...
  MasonryMode.prototype._manageStamp = function() {
    this.options.isOriginLeft = this.isotopeMB.options.isOriginLeft;
    this.options.isOriginTop = this.isotopeMB.options.isOriginTop;
...
FitRows.prototype._getItemLayoutPosition = function( item ) {
  ...
  if ( this.x !== 0 && item.size.outerWidth + this.x > this.isotopeMB.size.innerWidth ) {
    this.x = 0;
    this.y = this.maxY;
  }
...
// used for AMD definition and requires
function isotopeMBDefinition( Outlayer, getSize, matchesSelector, Item, LayoutMode ) {
  // create an Outlayer layout class
  ...
  IsotopeMB.prototype._create = function() {
    this.itemGUID = 0;
  ...
  IsotopeMB.prototype.reloadItems = function() {
  ...
  IsotopeMB.prototype._itemize = function() {
  ...
  IsotopeMB.prototype._initLayoutMode = function( name ) {
  ...
  IsotopeMB.prototype.layout = function() {
  ...
  IsotopeMB.prototype._layout = function() {
    // don't animate first layout
  ...
  // filter + sort + layout
  IsotopeMB.prototype.arrange = function( opts ) {
  ...
  IsotopeMB.prototype._init = IsotopeMB.prototype.arrange;

  // HACK
  // Don't animate/transition first layout
  // Or don't animate/transition other layouts
  IsotopeMB.prototype._getIsInstant = function() {
    var isInstant = this.options.isLayoutInstant !== undefined ?
      this.options.isLayoutInstant : !this._isLayoutInited;
  ...
  IsotopeMB.prototype._filter = function( items ) {
  ...
  IsotopeMB.prototype._getFilterTest = function( filter ) {
   ...
   * @public
   */
  IsotopeMB.prototype.updateSortData = function( elems ) {
    this._getSorters();
  ...
  IsotopeMB.prototype._getSorters = function() {
   ...
   * @private
   */
  IsotopeMB.prototype._updateItemsSortData = function( items ) {
    for ( var i=0, len = items.length; i < len; i++ ) {
      var item = items[i];
      item.updateSortData();
    }
  ...
  IsotopeMB.prototype._sort = function() {
  ...
  // get layout mode
  IsotopeMB.prototype._mode = function() {
  ...
  IsotopeMB.prototype._resetLayout = function() {
    // trigger original reset layout
    Outlayer.prototype._resetLayout.call( this );
    this._mode()._resetLayout();
  };

  IsotopeMB.prototype._getItemLayoutPosition = function( item  ) {
    return this._mode()._getItemLayoutPosition( item );
  };

  IsotopeMB.prototype._manageStamp = function( stamp ) {
    this._mode()._manageStamp( stamp );
  };

  IsotopeMB.prototype._getContainerSize = function() {
    return this._mode()._getContainerSize();
  };

  IsotopeMB.prototype.needsResizeLayout = function() {
  ...
  IsotopeMB.prototype.appended = function( elems ) {
    var items = this.addItems( elems );
    if ( !items.length ) {
      return;
    }
  ...
  IsotopeMB.prototype.prepended = function( elems ) {
    var items = this._itemize( elems );
    if ( !items.length ) {
      return;
    }
  ...
  IsotopeMB.prototype._filterRevealAdded = function( items ) {
   ...
   * @param {Array or NodeList or Element} elems
   */
  IsotopeMB.prototype.insert = function( elems ) {
    var items = this.addItems( elems );
    if ( !items.length ) {
      return;
    }
  ...
  var _remove = IsotopeMB.prototype.remove;
  IsotopeMB.prototype.remove = function( elems ) {
    elems = makeArray( elems );
    var removeItems = this.getItems( elems );
    // do regular thing
   ...
   * @private
   */
  IsotopeMB.prototype._noTransition = function( fn ) {
...
if ( typeof define === 'function' && define.amd ) {
  ...
  // browser global
  window.IsotopeMB = isotopeMBDefinition(
    window.Outlayer,
    window.getSize,
    window.matchesSelector,
